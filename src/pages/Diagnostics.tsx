...
import { AlertTriangle, HardDrive, Radio, Server, RefreshCw, XCircle } from 'lucide-react';
...
const Diagnostics: React.FC = () => {
    ...
    try {
      ...
      if ('serviceWorker' in navigator) {
        ...
        if (window.mcpBrowserToolsAPI && registration?.active) {
          try {
            const stats = await window.mcpBrowserToolsAPI.getServiceWorkerStats();
            setSwStats(stats);
          } catch (error) {
            console.error('Erro ao buscar estatísticas do Service Worker:', error);
          }
        }
      }
  ...
  return (
    <div className="py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className={`p-4 rounded-md mb-6 ${mcpConnected ? 'bg-green-50' : 'bg-yellow-50'}`}>
        <div className="flex">
          <div className={`flex-shrink-0 ${mcpConnected ? 'text-green-400' : 'text-yellow-400'}`}>
            {mcpConnected ? <Radio className="h-5 w-5" /> : <AlertTriangle className="h-5 w-5" />}
          </div>
          <div className="ml-3">
            <h3 className={`text-sm font-medium ${mcpConnected ? 'text-green-800' : 'text-yellow-800'}`}>
              {mcpConnected 
                ? 'MCP Browser Tools conectado' 
                : 'MCP Browser Tools não detectado'}
            </h3>
            <div className={`mt-2 text-sm ${mcpConnected ? 'text-green-700' : 'text-yellow-700'}`}>
              <p>
                {mcpConnected 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center">
            ...
            <h2 className="text-lg font-medium text-gray-900">Service Worker</h2>
          </div>
          ...
          <div className="mt-4 space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                swRegistration?.active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
              }`}>
                {getSwStatus()}
              </span>
            </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="mt-4 space-y-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className={`text-${errorCount > 0 ? 'red' : 'gray'}-900 font-medium`}>
                {errorCount}
              </span>
            </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-gray-900 font-medium">{networkRequestCount}</span>
            </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div className="border-t border-gray-200 px-4 py-5 sm:p-6">
          <div className="space-y-4">
            ...
            <p className="text-gray-700">
              Para acessar a extensão, clique no ícone do MCP Browser Tools na barra de extensões do navegador.
            </p>
          </div>
        </div>
      </div>
