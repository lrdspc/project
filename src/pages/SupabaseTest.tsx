  ...
  useEffect(() => {
    async function testConnection() {
      try {
        ...
        try {
          const clientsResult = await MCPSupabase.fetchRecords('clients', { limit: 1 });
        ...
        try {
          const inspectionsResult = await MCPSupabase.fetchRecords('inspections', { limit: 1 });
        ...
        try {
          const tilesResult = await MCPSupabase.fetchRecords('inspection_tiles', { limit: 1 });
        ...
        try {
          const photosResult = await MCPSupabase.fetchRecords('inspection_photos', { limit: 1 });
        ...
        try {
          const nonconformitiesResult = await MCPSupabase.fetchRecords('nonconformities', { limit: 1 });
          if (!nonconformitiesResult.error) availableTables.push('nonconformities');
        } catch (e) {
        ...
        setStatus('error');
        setMessage(`Erro na conexão com Supabase: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
        ...
        <div>
          ...
          {tables.length > 0 ? (
            ...
            <p className="text-amber-600">
              Nenhuma tabela disponível. Você precisa criar as tabelas no seu projeto Supabase.
            </p>
          )}
          ...
          <div className="mt-8 p-4 bg-gray-50 rounded-md">
            ...
            <ol className="list-decimal ml-6 space-y-2">
              <li>Se nenhuma tabela estiver disponível, você precisa criar as tabelas no seu novo projeto Supabase.</li>
              <li>Você pode encontrar os scripts de migração na pasta <code>supabase/migrations</code> deste projeto.</li>
              <li>Execute esses scripts no SQL Editor do seu projeto Supabase para criar a estrutura necessária.</li>
            </ol>
          </div>
        </div>
      )}
